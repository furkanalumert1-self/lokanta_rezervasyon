        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
          ...
          <div className="flex gap-2">
            <input type="text" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())}
              onKeyDown={(e) => e.key === "Enter" && handleSearch()} placeholder="Örn: RZ4K8M" maxLength={6}
              className="flex-1 border border-gray-200 rounded-xl px-4 py-3 text-sm font-mono tracking-widest uppercase focus:outline-none focus:ring-2 focus:ring-primary-300" />
            <button onClick={handleSearch} disabled={loading || code.length < 4}
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={row.label} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  <div className="flex items-center gap-2 text-sm text-gray-500"><row.icon className="w-4 h-4" />{row.label}</div>
                  <span className="text-sm font-medium text-gray-900">{row.value}</span>
                </div>
              ))}
              {result.notes && <div className="pt-1"><p className="text-xs text-gray-400 mb-1">Not</p><p className="text-sm text-gray-700">{result.notes}</p></div>}
            </div>
          </div>
        )}
      </div>
